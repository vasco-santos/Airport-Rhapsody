...
static unsigned int whatShouldIDo (unsigned int k, unsigned int id)
{
	...
	// If she is the very first passenger, she sets the number of the plane landing
	if (sh->nPassP == 0)
	{
		sh->fSt.nLand++;
		// State should only be saved by the very first passenger descending from the plane.
	...
	sh->nPassP++;
	// If she is the very last passenger descending from the plane, she must inform the porter
	// that a new plane has landed.
	if (sh->nPassP == N)
	{
		if (semUp (semgid, sh->waitingFlight) == -1)
		{
			perror ("error on the up operation for semaphore waitingFlight (PA)");
			exit (EXIT_FAILURE);
		}
	}
	/* The passenger descending from the plane decides what should she do according to her situation
	 * (has this airport as her final destination and has bags to collect / has this airport as her
	 *  final destination with no bags to collect / is in transit) and updates statistical information. */
	if (sh->fSt.st.passStat[k][id].sit == FD)
	{
		// Udpdate total number of passengers for whom the airport was their final destination
		sh->fSt.nToTPassFD++;
	...
	{
		// Update total number of passengers in transit
		sh->fSt.nToTPassTST++;
	}
	saveState (nFic,k,&(sh->fSt));
	// else if (sh->fSt.st.passStat[k][id].sit == TRT)
...
void goHome (unsigned int k, unsigned int id)
{
	...
	// if she is the very last passenger of the very last flight - Wake Up Others
	int i;
	for (i = 0; i < N; i++)
	{
		if (sh->fSt.st.passStat[k][i].stat == EXITING_THE_ARRIVAL_TERMINAL || sh->fSt.st.passStat[k][i].stat == ENTERING_THE_DEPARTURE_TERMINAL)
		{
			count++;
		}
	}
	if (count == N)
	{
		...
		// if she is the very last passenger of the very last flight, she informs the
		// bus driver that his day's work is finished
		if (k == K-1)
		{
	...
	// She waits for all other passengers being ready to either exit the airport
	if (count != N)
	{
		if (semDown (semgid, sh->pass[id]) == -1)
		{
			perror ("error on the down operation for semaphore pass[id] (PA)");
			exit (EXIT_FAILURE);
		}
	}
}
...
static void takeABus (unsigned int k, unsigned int id)
{
	...
	// Adiciona a queue o passageiro

	queueIn(&sh->fSt.busQueue, id);
	/* she informs the bus driver it is time to start boarding if the number of queueing
	 *  passengers is equal to the number of seats in the bus */
	int i;
	int count = 0;
	for (i = 0; i < N; i++)
	{
		if (sh->fSt.st.passStat[k][i].stat == AT_THE_ARRIVAL_TRANSFER_TERMINAL)
		{
			count++;
		}
	}
	...
	if (semDown (semgid, sh->pass[id]) == -1)
	{
		perror ("error on the up operation for semaphore passanger (PA)");
		exit (EXIT_FAILURE);
	}
}
...
static void enterTheBus (unsigned int k, unsigned int id)
{
	...
	sh->fSt.bus.seat[sh->fSt.bus.nOccup++] = id;
	sh->nPassD++;
	// Change State
	...
	/* Wake Up Driver */
    /* insert your code here */
	if (sh->fSt.bus.nOccup == T || queueEmpty(&sh->fSt.busQueue))
	{
...
static void leaveTheBus (unsigned int k, unsigned int id)
{
	...
	/* insert your code here */
	// The passenger checks the bus occupation.
	...
	sh->fSt.bus.seat[sh->fSt.bus.nOccup--] = EMPTYPOS;
	sh->nPassD++;
	saveState (nFic,k,&(sh->fSt));
